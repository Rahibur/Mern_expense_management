...
const Analytics = ({ allTransaction }) => {
    ...
    const categories = ['salary', 'tip', 'project', 'food', 'movie', 'medical', 'fee', 'tax'];
    ...
    const totalIncomeTurnover = allTransaction.filter(
        (transacrion) => transacrion.type === 'income').reduce((acc, transacrion) => acc + transacrion.amount, 0);

    const totalExpenseTurnover = allTransaction.filter(
        (transacrion) => transacrion.type === 'expense').reduce((acc, transacrion) => acc + transacrion.amount, 0);
        ...
        <>
            ...
            <div className='row mt-3'>
                <div className='col-md-4'>
                    <h4>Category Income</h4>
                    {
                        categories.map(category => {
                            const amount = allTransaction.filter(transacrion => transacrion.type === 'income'
                                && transacrion.category === category).reduce((acc, transacrion) => acc + transacrion.amount, 0);
                            return (
                                amount > 0 && (
                                    <div className='card'>
                                        <div className='card-body'>
                ...
                <div className='col-md-4'>
                    ...
                    {
                        categories.map(category => {
                            const amount = allTransaction.filter(transacrion => transacrion.type === 'expense'
                                && transacrion.category === category).reduce((acc, transacrion) => acc + transacrion.amount, 0);
                            return (
                                amount > 0 && (
                                    <div className='card'>
